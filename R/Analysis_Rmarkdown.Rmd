...
<!-- # # Weighted response rate -->
<!-- wt.rr <- by(data = data.frame(resp = as.numeric(unlist(allLarges.requested[, "response"])),  -->
<!--                                wt = as.numeric(unlist(allLarges.requested[,"base.weight"]))), -->
...
<!-- # tmp2 <- data.frame(cbind(NR.class=as.numeric(names(wt.rr)), unwt.rr[,2], wt.rr)) -->
...
<!--             by = "PWSID") -->
<!-- allLarges %<>% mutate(final.weight.nonresponse = unwt.rr.final * base.weight) -->
...
#Save to csv
write.csv(x = larges %>% 
             select(PWSID,`weights(ps.dsgn.larges)`),
           file = "Datasets/fullSurveyWEIGHTS.csv"
           )
```
