...
allSmalls.volunteers <- allSmalls.requested.responded <- allSmalls %>%  filter((voluntary == "y"))
#Dataset for all responses (volunteers and requested)
allSmalls.responded <- allSmalls %>% 
  filter(responded == "y")
```
# Examining Completion of Questions
...
## Mice Implementation
First let's start my building an Ordinary Least Squares (OLS) linear regression to predict cash in reserve (total) based on relevant predictors for all systems (e.g. service connections, median 12 month household income, delinquent number of accounts, delinquent amout in dollars, CalEnvrioScreen 3 scores, volunteer status for filling out survey). We will ignore the volunteer data for now. 
```{r}
## Estimate an OLS Regression
fitols <- lm(months_before_assist_num ~ Service_Connections + Population + cash_reserve_total + Median_12month_HH_income + Median_rent_pct_income + CES_3.0_Score + delinquent_num_acc + volunteered + delinquent_amount_dollars, data = allSmalls.requested.responded)
summary(fitols)
```
As we can see in the table above, a significant number of observations were deleted due to missingness (80 out of 371). Since this is greater than 14% of the whole dataset, our survey would be limited in power.
...
allSmalls.simple <- allSmalls.requested.responded %>% 
  select(PWSID, Service_Connections, Population, Median_12month_HH_income, Median_12month_HH_income, CES_3.0_Score, months_before_assist, cash_reserve_total, delinquent_num_acc, delinquent_amount_dollars, revenue_2020_Total, revenue_2019_Total)

#split the other half of the dataset for easy joining
allSmalls.rest <- allSmalls.requested.responded %>% 
  select(-Service_Connections, -Population, -Median_12month_HH_income, -Median_12month_HH_income, -CES_3.0_Score, -months_before_assist, -cash_reserve_total, -delinquent_num_acc, -delinquent_amount_dollars, -revenue_2020_Total, -revenue_2019_Total)
#see missing values
...
simpleImputed <- complete(imp2) %>% select(-CES_3.0_Score, -Median_12month_HH_income, -months_before_assist, -Service_Connections, -Population)
