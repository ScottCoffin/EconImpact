...
allSmalls.requested.responded %>% 
  select(PWSID, Service_Connections,  Median_12month_HH_income, CES_3.0_Score,
         ...
         revenue_2020_Total, revenue_2019_Total, cash_reserve_unrestricted  , expense_2019_Apr , expense_2019_May , expense_2019_Jun , expense_2019_Jul , expense_2019_Aug , expense_2019_Sep , expense_2019_Oct , expense_2019_Total , expense_2020_Apr , expense_2020_May , expense_2020_Jun , expense_2020_Jul , expense_2020_Aug , expense_2020_Sep , expense_2020_Oct , expense_2020_Total) %>% 
...
plot(ini)
```
Convergence is quite good for some variables (revenue  2020 and 2019 totals,cash reserves), but poor for others. CalEnviroScreen 3.0 score imputation looked reasonable. The imputations are annotated below in red, with observed values in blue.
...
densityplot(ini)
```
Many of these variables seem to be well-predicted by the mode. The worst seems to be CES 3.0 score and 12 month household income. Looking closer at these distributions.
```{r}
densityplot(ini, ~CES_3.0_Score)
```
We can see here that the quick prediction model biases towards lower CalEnviroScreen 3.0 scores. Let's see if the classification and regression tree (CART) model worked any better.
```{r}
densityplot(imp2, ~CES_3.0_Score)
```

The CART method seems to have learned the distribution slightly more accurately for this variable. The most complex distribution is months before assistance needed. Let's see how well the passive method learned this.
```{r}
...
Let's further compare the default imputation method (passive multiple imputation) with the quick prediction method and the Classification and regression trees method for CES 3.0 score. 
```{r}
...
```
Here we see that these three methods do not seemingly differ in prediction. Any model would be acceptable for imputation.
...
```{r join larges imputed data,results= 'hide'}
...
Let's define our survey structure in the *survey* package.
```{r survey initiate LARGE, results='hide'}
##### Specify sampling design ####
#here we will use our imputed survey dataset
#(stratified)
...
There are no missing adjusted weights, calibration may proceed.
...
ggplot(aes(x = calibration, y = total, 
  ...
  theme(
        ...
        plot.subtitle = element_text(hjust = 0.5))
```
The impact of weigthing (and adjusting weights using calibration) on both estimate totals and error can be seen above.A simpler figure is below.
...
Calculated risk score (derived by **Marielle**), is calculated and plotted below for small systems.
Totals can be visualized graphically.
...
```
 ...
 These data can also be viewed by fee code. 
...
These data are reported with error bars (not grouped) below. 
...
#### *Perceived* Risk
The perceived risk, reported by system interviewees in response to the question "how many months do you estimate that you will be able to go for needing finanical assistance?" with discrete responses labeled "A - F," corresponding the followingresponses:
A: 0-3 months 
B: 4-6 months 
C: 7-9 months 
D: 10-12 months 
...
ggplot(aes(x = Risk, y = total, 
  ...
  theme(legend.position = "none",
    ...
    plot.caption = element_text(size = 8))
```
Proportions of total systems in these categories are plotted below.
...
#### Correlation: *Percieved* vs. Estimated Risk 

```{r}
allSmalls.complete %>% 
  ggplot(aes(x = risk_score_avg, y =months_before_assist_num, weights= final.weight, color = Fee_Code)) +
  geom_point()
```
...
Below is for larges for total dollars of delinquent accouts (including other expenses such as power).
```{r}
#larges
...
dollars_del_acc_TotRs_col %<>% mutate(year = "2020")

#Construct a barplot
dollars_del_acc_TotRs_col %>% 
  mutate(Fee_Code = fct_reorder(Fee_Code, dollars_del_acc_TotR)) %>%  #reoder so davcl is on bottom
ggplot(aes(x = year, y = dollars_del_acc_TotR, 
                     ymin = lower, ymax = upper, fill = Fee_Code)) +
...
```{r}
#larges
debt.total <- svytotal(~dollars_del_acc_TotR, 
         design = ps.dsgn.larges,
         na.rm = TRUE) %>% 
  data.frame() %>% 
  mutate(Debt = "All Expenses") %>% 
  rename(c("SE" = dollars_del_acc_TotR))
debt.total
```
...
```{r}
#larges
dw.debt.total <- svytotal(~dollars_del_dw_TotR, 
         design = ps.dsgn.larges,
         na.rm = TRUE) %>% 
  data.frame() %>% 
  mutate(Debt = "Drinking Water Expenses") %>% 
  rename(c("SE" = dollars_del_dw_TotR))
dw.debt.total
```

Totals are depicted graphically below.
```{r}
#join dataframes
debt_larges <- rbind(dw.debt.total, debt.total) %>% 
  data.frame()

#add error bars
debt_larges_comparison <- mutate(debt_larges,
                                 lower = total - 1.96*SE, #95% CI
...
#Construct a barplot
debt_larges_comparison %>% 
ggplot(aes(x = Debt, y = total, 
                     ymin = lower, ymax = upper, fill = Debt)) +
  geom_col(position = "identity") +
  scale_fill_manual(values = cal_palette("chaparral2"))+
  scale_x_discrete(name = "Debt Type") +
  scale_y_continuous(labels = scales::dollar_format(),
                     limits = c(0,1000000000)) +
  labs(y = "Statewide Estimated Debt ($USD)") +
  geom_errorbar(width = 0.7) +
  labs(title = "Systems >10,000 Service Connections",
       subtitle = "(95 Confidence Interval)",
       caption = "Post-stratified by service connections and median household income") +
