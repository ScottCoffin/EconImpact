...
def input_and_refresh(model_work,mod_refresh,test_inputs):
    ...
    wb = xlapp.Workbooks.Open(model_mod)
    wb.RefreshAll()
...
model_work='Alternativesurveyfrequency_WaterLossModel_21August2020_V3.xlsx'
#Read the file that contains the inputs for the various suppliers (extracted from water loss audits)
inputs=pd.read_excel('Audit_inputs_2020-07-06.xlsx')
...
output_subdir=r'S:\EXECUTIVE\OSI\Water Loss\MODEL\{:}'.format(vn)
if not os.path.exists(output_subdir):
    os.makedirs(output_subdir)

#Where the summary files will be stored
#PROBABLY WANT TO CHANGE THIS TO A LOCAL DIRECTORY
sub_summ_dir=r'S:\EXECUTIVE\OSI\Water Loss\MODEL\Summaries\{:}'.format(vn)
...
mod_refresh=r'{:}/model_calculations_refresh.xlsx'.format(dirn)   
...
for i in range(0,len(inputs)):
    ...
    mod_refresh_rev=r'{:}/model_calculations_{:}_21August2020_V3_AlternativeSurveyFrequency.xlsx'.format(output_subdir,snf)
...
w=pd.ExcelWriter('{:}/model_summary_standards_AlternativeSurveyFrequency.xlsx'.format(sub_summ_dir))
